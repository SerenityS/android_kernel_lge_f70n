...
#if defined(CONFIG_MACH_MSM8926_F70N_KR)
#define SMALLZ_SCALE_00_REG				0x49
#define SMALLZ_SCALE_01_REG				0x4A
#endif
...
#if	defined(CONFIG_MACH_MSM8926_F70N_KR)
	buf = 0x2b;
	if (touch_i2c_write(client, SMALLZ_SCALE_00_REG, 1, &buf) < 0){
		TOUCH_ERR_MSG("SMALLZ_SCALE_00_REG write fail\n");
		return -EIO;
	}
	buf = 0x17;
	if (touch_i2c_write(client, SMALLZ_SCALE_01_REG, 1, &buf) < 0){
		TOUCH_ERR_MSG("SMALLZ_SCALE_01_REG write fail\n");
		return -EIO;
	}
#endif
...
int synaptics_ts_fw_upgrade(struct i2c_client* client, struct touch_fw_info* fw_info)
{
	...
	if ((ret = request_firmware((const struct firmware **) (&fw_info->fw_upgrade.fw), fw_path, &client->dev)) != 0) {
